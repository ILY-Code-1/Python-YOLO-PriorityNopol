...
class OCRService:
    ...
    def _preprocess(self, image_bgr: np.ndarray) -> np.ndarray:
        ...
        if h > 0:
            ...
            image_bgr = cv2.resize(image_bgr, (new_w, 64), interpolation=cv2.INTER_CUBIC)
    ...
    def read_plate(self, image_crop_bgr: np.ndarray) -> str:
        ...
        # Kumpulkan token dengan confidence >= 0.10
        tokens = [text for (_, text, conf) in ocr_results if conf >= 0.10]
